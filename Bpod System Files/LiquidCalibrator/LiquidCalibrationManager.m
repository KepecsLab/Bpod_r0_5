...
if ~isfield(handles, 'PendingMeasurements') % The OpeningFcn is being called for the first time (is also called whenever handles are imported to another function. WTF, MATLAB???)
    ...
    if exist(CalibrationFilePath) ~= 2
        ...
        CalibrationFilePath = fullfile(BpodPath, 'Bpod System Files','LiquidCalibrator', 'CalibrationFileTemplate.mat');
    end
...
global Measurement2add % Because MATLAB GUI sucks
...
if isPendingMeasurement == 0
    ...
    ValveData = handles.LiquidCal(CurrentValve).Table;
    Coeff = handles.LiquidCal(CurrentValve).TrinomialCoeffs;
    ...
    if nMeasurements > 1
        ...
        if (nMeasurements) > 2
            handles.LiquidCal(CurrentValve).TrinomialCoeffs = polyfit(handles.LiquidCal(CurrentValve).Table(:,2),handles.LiquidCal(CurrentValve).Table(:,1),3);
        elseif nMeasurements > 1
            handles.LiquidCal(CurrentValve).TrinomialCoeffs = polyfit(handles.LiquidCal(CurrentValve).Table(:,2),handles.LiquidCal(CurrentValve).Table(:,1),2);
        else
            handles.LiquidCal(CurrentValve).TrinomialCoeffs = [];
        end
        warning on
...
if nValidValves > 0
    ...
    for x = 1:nValidValves
        ...
        p = ValveData(ValveList(x)).TrinomialCoeffs;
        if ~isempty(p)
