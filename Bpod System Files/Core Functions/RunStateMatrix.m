...
fwrite(BpodSystem.SerialPort, 'R'); % Send the code to run the loaded matrix (character "R" for Run)
...
while MatrixFinished == 0
    if BpodSystem.SerialPort.BytesAvailable > 0
        nCurrentEvents = fread(BpodSystem.SerialPort, 1);
        TempCurrentEvents = fread(BpodSystem.SerialPort, nCurrentEvents);
...
if BpodSystem.BeingUsed == 1
    ...
    % Accept Timestamps
    TrialStartTimestamp = fread(BpodSystem.SerialPort, 1, 'uint32')/1000; % Start-time of the trial in milliseconds (immune to 32-bit clock rollover)
    TrialStartMicroseconds = fread(BpodSystem.SerialPort, 1, 'uint32'); % Time the trial started (in microseconds)
    TrialStartMilliseconds = TrialStartMicroseconds/1000;
    nTimeStamps = fread(BpodSystem.SerialPort, 1, 'uint16');
    TimeStamps = zeros(1,nTimeStamps);
    for x = 1:nTimeStamps
        TimeStamps(x)  = fread(BpodSystem.SerialPort, 1, 'uint32');
    end
